...
class acrossSession(object):
    ...
    def __init__(self, subjects, nr_sessions, base_dir, split_by='pupil_d'):
        ...
        self.right = []
        for i in range(len(self.subjects)):
            if self.subjects[i] in ['DE', 'DL', 'JG', 'LH', 'LP', 'NS', 'TK']:
    ...
    def simulation(self):
        ...
        XX = m[0]
        YY = m[1]
        MM = XX + (np.random.rand(len(XX)) * 50)
        MM = myfuncs.lin_regress_resid(MM, [YY]) + MM.mean()
        
        # get bins:
        n_bins = 10
        inds_s = np.zeros((len(XX), n_bins), dtype=bool)
        bin_measure = XX
        inds = np.array_split(np.argsort(bin_measure), n_bins)
        for b in range(n_bins):
            ind = np.zeros(len(bin_measure), dtype=bool)
        ...
        M = np.zeros(n_bins)
        for b in range(n_bins):
            X[b] = XX[inds_s[:,b]].mean()
            Y[b] = YY[inds_s[:,b]].mean()
            M[b] = MM[inds_s[:,b]].mean()
        
        fig = plt.figure(figsize=(3,3))
        ax = fig.add_subplot(111)
        myfuncs.correlation_plot(XX,YY,line=True,ax=ax)
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        plt.tight_layout()
        ...
        fig = plt.figure(figsize=(3,3))
        ax = fig.add_subplot(111)
        myfuncs.correlation_plot(XX,MM,line=True,ax=ax)
        ax.set_xlabel('X')
        ax.set_ylabel('M')
        plt.tight_layout()
        ...
        fig = plt.figure(figsize=(3,3))
        ax = fig.add_subplot(111)
        myfuncs.correlation_plot(MM,YY,line=True,ax=ax)
        ax.set_xlabel('M')
        ax.set_ylabel('Y')
        plt.tight_layout()
        ...
        fig = plt.figure(figsize=(3,3))
        ax = fig.add_subplot(111)
        myfuncs.correlation_plot(M,Y,line=True,ax=ax)
        ax.set_xlabel('M')
        ax.set_ylabel('Y')
        plt.tight_layout()
        plt.show()
        
        c, c_p, a, b1, b2 = myfuncs.mediation_analysis(X,Y,M, model='ols')
        direct = c.copy()
        direct_p = c_p.copy()
        indirect = a*(b1+b2)
        print 'direct effect = {}'.format(direct_p)
        print 'indirect effect = {}'.format(indirect)
        
        
        
        c, c_p, a, b1, b2 = myfuncs.mediation_analysis(XX,YY,MM, model='ols')
        direct = c.copy()
        direct_p = c_p.copy()
        indirect = a*(b1+b2)
        
    def CHOICE_SIGNALS_PPI_analysis(self,):
        ...
        #         'X2' : pd.Series(dfs_brainstem[i]['mean_SN_d']),
        #         'X3' : pd.Series(dfs_brainstem[i]['mean_VTA_d']),
        #         'X4' : pd.Series(dfs_brainstem[i]['basal_forebrain_d']),}
        #     data = pd.DataFrame(d)
        ...
        #         higher_cutoff = np.percentile(pupil_b+pupil_d, 90)
        #         # ceiling effect control:
        #         omissions.append((pupil_b+pupil_d > higher_cutoff) + (pupil_b+pupil_d < lower_cutoff))
        #
        #     omissions = np.concatenate(omissions)
        #     omissions_s.append(omissions)
        #
        # dfs = [dfs[i][~omissions_s[i]] for i in range(len(self.subjects))]
        # self.pupil_data = [self.pupil_data[i][~omissions_s[i]] for i in range(len(self.subjects))] 
        
        # shell()
        
        # rois = ['FS_S_intrapariet_and_P_trans_mean', 'FS_S_intrapariet_and_P_trans_info',]
        # roi_names = ['IPS_m', 'IPS_i', ]
        
        # rois = ['V1', 'V2', 'V3', 'V1_center_info', 'V2_center_info', 'V3_center_info', 'lr_aIPS_lat', 'lr_PCeS_lat', 'lr_M1_lat',]
        # roi_names = ['V1_td', 'V2_td', 'V3_td', 'V1_s', 'V2_s', 'V3_s', 'aIPS', 'IPS/PCeS', 'M1',]
        
        # rois = ['V2', 'V3', 'lr_PCeS_lat', 'lr_M1_lat',]
        # roi_names = ['V2_td', 'V3_td', 'IPS/PCeS', 'M1',]
        
        rois = ['V1_center_info_choice', 'V2_center_info_choice', 'sl_IPL_info', 'sl_SPL1_info', 'lr_PCeS_lat', 'sl_pIns_info', 'combined_2']
        roi_names = ['V1', 'V2', 'IPL', 'SPL', 'IPS/PCeS', 'pIns', 'combined']
        
        M_measures = ['pupil_d', 'LC_JW_d', 'BS_COCKTAIL', 'LC_ACC']
        
        M_measure = M_measures[0]
        ...
        for version in [1]:
            ...
            for roi in rois:
                ...
                if version == 1:
                    ...
                    # run analysis:
                    direct = np.zeros(len(self.subjects))
                    cc = np.zeros(len(self.subjects))
                    interaction = np.zeros(len(self.subjects))
                    for i in range(len(self.subjects)):
                        Y = np.array(self.yes[i], dtype=int)
                        try:
                            M = np.array(self.pupil_data[i][M_measure])
                        except:
                            M = np.array(dfs_brainstem[i][M_measure])
                        try:
                            X = np.array(dfs[i][roi])
                        except:
                            X = np.array(dfs_cortex[i][roi])
                        
                        c, c_p, b = myfuncs.PPI_analysis(X,Y,M, model='logit',)
                        direct[i] = c
                        cc[i] = c_p
                        interaction[i] = b
                        ...
                        YY.append(Y)
                        MM.append(M)
                        S.append(np.ones(len(X))*(i+1))
                
                    # save and print:
                    roi_c.append(cc)
                    roi_interaction.append(interaction)
                    roi_direct.append(direct)
                    # print
                    # print 'Direct effect:'
                    # print myfuncs.permutationTest(direct, np.zeros(len(self.subjects)))
                    # print 'Direct effect, controlling for M:'
                    # print myfuncs.permutationTest(direct_p, np.zeros(len(self.subjects)))
                    # print 'Indirect effect:'
                    # print myfuncs.permutationTest(indirect, np.zeros(len(self.subjects)))
                    # print
                    #
                    # # shell()
                    #
                    # d = {'X' : pd.Series(np.concatenate(XX)),
                    #     'Y' : pd.Series(np.concatenate(YY)),
                    #     'M' : pd.Series(np.concatenate(MM)),
                    #     's' : pd.Series(np.concatenate(S))}
                    # data = pd.DataFrame(d)
                    # data.to_csv(os.path.join(self.figure_folder, 'mediation_data_all_trials.csv'))
                
                # random effects binned:
                # ---------------------
                elif version == 2:
                    ...
                    for i in range(len(self.subjects)):
                        ...
                        for s in np.array(np.unique(self.pupil_data[i]['session']), dtype=int):
                            rt = np.array(self.pupil_data[i]['rt'])[np.array(self.pupil_data[i].session == s)]
                            try:
                                X = np.array(dfs[i][roi])[np.array(self.pupil_data[i].session == s)]
                            except:
                                X = np.array(dfs_cortex[i][roi])[np.array(self.pupil_data[i].session == s)]
                            nr_trials_in_run = len(X)
                            inds = np.array_split(np.argsort(X), n_bins)
                            for b in range(n_bins):
                        ...
                        M = np.zeros(n_bins)
                        for b in range(n_bins):
                            data = self.pupil_data[i][inds_s[:,b]]
                            try:
                                M[b] = data[M_measure].mean()
                            except:
                                M[b] = np.array(dfs_brainstem[i][M_measure])[inds_s[:,b]].mean()
                            try:
                                X[b] = np.array(dfs[i][roi])[inds_s[:,b]].mean()
                            except:
                                X[b] = np.array(dfs_cortex[i][roi])[inds_s[:,b]].mean()
                            d, c = myfuncs.SDT_measures(target=np.array(data.present), hit=np.array(data.hit), fa=np.array(data.fa))
                            # c = np.array(data.yes).sum() / float(len(np.array(data.yes)))
                            Y[b] = c
                        
                        c, c_p, b = myfuncs.PPI_analysis(X,Y,M, model='ols',)
                        direct[i] = c
                        cc[i] = c_p
                        interaction[i] = b
                    ...
                    #         's' : pd.Series(np.concatenate(S))}
                    #     data = pd.DataFrame(d)
                    #     data.to_csv(os.path.join(self.figure_folder, 'mediation_data_binned.csv'))
                ...
                #                 bin_measure = myfuncs.lin_regress_resid(bin_measure, [rt])
                #             else:
                #                 bin_measure = np.array(dfs[i][roi])[np.array(self.pupil_data[i].session == s)]
                #             nr_trials_in_run = len(bin_measure)
                #             inds = np.array_split(np.argsort(bin_measure), n_bins)
                #             for b in range(n_bins):
                #                 ind = np.zeros(len(bin_measure), dtype=bool)
                ...
                #
                #     # run analysis:
                #     X = np.zeros((len(self.subjects), n_bins))
                #     Y = np.zeros((len(self.subjects), n_bins))
                #     M = np.zeros((len(self.subjects), n_bins))
                #     for i in range(len(self.subjects)):
                #         for b in range(n_bins):
                #             data = self.pupil_data[i][bins[i][:,b]]
                #             X[i,b] = data['pupil_d'].mean()
                #             M[i,b] = BOLD_subjects[i][bins[i][:,b]].mean()
                #             d, c = myfuncs.SDT_measures(target=np.array(data.present), hit=np.array(data.hit), fa=np.array(data.fa))
                #             # c = np.array(data.yes).sum() / float(len(np.array(data.yes)))
                #             Y[i,b] = c
                #     X = X.mean(axis=0)
                ...
                #     # save and print:
                #     roi_direct.append(direct)
                #     roi_direct_p.append(direct_p)
                #     roi_indirect.append(indirect)
                #     print 'Direct effect:'
                #     print direct
                #     print
                #     print 'Direct effect, controlling for M:'
            ...
            # plotting:
            # ---------    
            roi_c = np.array(roi_c).T
            roi_interaction = np.array(roi_interaction).T
            roi_direct = np.array(roi_direct).T
            titles = ['c', 'interaction', 'direct']
            measures = [roi_c, roi_interaction, roi_direct]
            for measure, title in zip(measures, titles):
                ...
                for r, roi in enumerate(rois):
                    ...
                    try:
                        plt.bar(locs[r], measure[:,r].mean(), width = bar_width, yerr = sp.stats.sem(measure[:,r]), color = 'gray', alpha = 1, align = 'center', edgecolor = 'k', linewidth = 0, capsize = 0, ecolor = 'k', error_kw={'elinewidth':0.5,})
                        # p1 = myfuncs.permutationTest(measure[:,r], np.zeros(measure.shape[0]), paired=True)[1]
                        p1 = sp.stats.wilcoxon(measure[:,r], np.zeros(measure.shape[0]))[1]
                        if p1 < 0.05:
                            plt.text(s='{}'.format(round(p1, 3)), x=locs[r], y=gca().get_ylim()[0]+((gca().get_ylim()[1] - gca().get_ylim()[0]) / 8.0), size=5, horizontalalignment='center', rotation=90)
                    except:
                        plt.bar(locs[r], measure[r].mean(), width = bar_width, color = 'gray', alpha = 1, align = 'center', edgecolor = 'k', linewidth = 0, capsize = 0, ecolor = 'k', error_kw={'elinewidth':0.5,})
                plt.title(title)
                sns.despine(offset=10, trim=True)
                plt.xticks(locs, roi_names, rotation=45)
                plt.tight_layout()
                fig.savefig(os.path.join(self.figure_folder, 'V123', 'cp', 'PPI_{}_{}.pdf'.format(version, title)))
            
            # # shell()
            # nr_subjects = len(self.subjects)
            # dv = np.concatenate((roi_interaction[:,0], roi_interaction[:,1], roi_interaction[:,2]))
            # area = np.concatenate((np.zeros(nr_subjects), np.ones(nr_subjects)*1, np.ones(nr_subjects)*2,))
            # subject = np.concatenate((np.arange(nr_subjects), np.arange(nr_subjects), np.arange(nr_subjects)))
            # d = rlc.OrdDict([('area', robjects.FactorVector(list(area.ravel()))), ('subject', robjects.FactorVector(list(subject.ravel()))), ('data', robjects.FloatVector(list(dv.ravel())))])
            # robjects.r.assign('dataf', robjects.DataFrame(d))
    ...
    def OLD_CHOICE_SIGNALS_mediation_analysis(self):
        ...
        # rois = ['sl_IPL_info', 'sl_SPL1_info', 'lr_PCeS_lat', 'sl_pIns_info', 'combined_1']
        # roi_names = ['IPL', 'SPL', 'IPS/PCeS', 'pIns', 'Combined']
        
        rois = ['V1_center_info_choice', 'V2_center_info_choice', 'sl_IPL_info', 'sl_SPL1_info', 'lr_PCeS_lat', 'sl_pIns_info', 'combined_2']
        roi_names = ['V1', 'V2', 'IPL', 'SPL', 'IPS/PCeS', 'pIns', 'Combined']
        
        X_measures = ['pupil_d', 'LC_JW_d', 'BS_COCKTAIL', 'LC_ACC']
        
        X_measure = X_measures[0]
        ...
        for version in [1]:
            ...
            for roi in rois:
                ...
                if version == 1:
                    ...
                    # run analysis:
                    direct = np.zeros(len(self.subjects))
                    aa = np.zeros(len(self.subjects))
                    bb1 = np.zeros(len(self.subjects))
                    bb2 = np.zeros(len(self.subjects))
                    cc = np.zeros(len(self.subjects))
                    indirect = np.zeros(len(self.subjects))
                    simple_indirect = np.zeros(len(self.subjects))
                    bbic1 = np.zeros(len(self.subjects))
                    bbic2 = np.zeros(len(self.subjects))
                    for i in range(len(self.subjects)):
                        Y = np.array(self.yes[i], dtype=int)
                        try:
                            X = np.array(self.pupil_data[i][X_measure])
                        except:
                            X = np.array(dfs_brainstem[i][X_measure])
                        try:
                            M = np.array(dfs[i][roi])
                        except:
                            M = np.array(dfs_cortex[i][roi])
                        a, b1, c_p, c, bic1, bic2 = myfuncs.mediation_analysis(X,Y,M, eq1='ols', eq2='logit', simple=True)
                        simple_indirect[i] = a*b1
                        # a, b1, b2, c_p, c, bic1, bic2 = myfuncs.mediation_analysis(X,Y,M, eq1='ols', eq2='logit', simple=False)
                        # bb2[i] = b2
                        # indirect[i] = a*(b1+b2)
                        direct[i] = c
                        aa[i] = a
                    ...
                    roi_c.append(cc)
                    roi_indirect.append(indirect)
                    roi_simple_indirect.append(simple_indirect)
                    roi_direct.append(direct)
                    roi_bic1.append(bbic1)
                    roi_bic2.append(bbic2)
                    # print
                    # print 'Direct effect:'
                    # print myfuncs.permutationTest(direct, np.zeros(len(self.subjects)))
                    # print 'Direct effect, controlling for M:'
                ...
                # ---------------------
                    ...
                    for i in range(len(self.subjects)):
                        ...
                        #     rt = np.array(self.pupil_data[i]['rt'])[np.array(self.pupil_data[i].session == s)]
                        #     try:
                        #         X = np.array(self.pupil_data[i][X_measure])[np.array(self.pupil_data[i].session == s)]
                        #     except:
                        #         try:
                        #             X = np.array(dfs[i][X_measure])[np.array(self.pupil_data[i].session == s)]
                        #         except:
                        #             X = np.array(dfs_brainstem[i][X_measure])[np.array(self.pupil_data[i].session == s)]
                        #     nr_trials_in_run = len(X)
                        #     inds = np.array_split(np.argsort(X), n_bins)
                        #     for b in range(n_bins):
                        #         ind = np.zeros(len(X), dtype=bool)
                        ...
                        for b in range(n_bins):
                            data = self.pupil_data[i][inds_s[:,b]]
                            try:
                                X[b] = data['pupil_d'].mean()
                            except:
                                X[b] = np.array(dfs_brainstem[i][X_measure])[inds_s[:,b]].mean()
                            try:
                                M[b] = np.array(dfs[i][roi])[inds_s[:,b]].mean()
                            except:
                                M[b] = np.array(dfs_cortex[i][roi])[inds_s[:,b]].mean()
                            d, c = myfuncs.SDT_measures(target=np.array(data.present), hit=np.array(data.hit), fa=np.array(data.fa))
                            # c = np.array(data.yes).sum() / float(len(np.array(data.yes)))
                            Y[b] = c
                        b2 = 0
                        a, b1, c_p, c = myfuncs.mediation_analysis(X,Y,M, model='ols', simple=True)
                        # a, b1, b2, c_p, c = myfuncs.mediation_analysis(X,Y,M, model='ols', simple=False)
                        direct[i] = c
                        aa[i] = a
                        bb1[i] = b1
                        bb2[i] = b2
            ...
            # plotting:
            # ---------    
            roi_a = np.array(roi_a).T
            roi_b1 = np.array(roi_b1).T
            roi_b2 = np.array(roi_b2).T
            roi_c = np.array(roi_c).T
            roi_indirect = np.array(roi_indirect).T
            roi_simple_indirect = np.array(roi_simple_indirect).T
            roi_direct = np.array(roi_direct).T
            roi_bic1 = np.array(roi_bic1).T
            roi_bic2 = np.array(roi_bic2).T
            titles = ['a', 'b1', 'b2', 'c', 'indirect', 'direct', 'simple_indirect', 'bic1', 'bic2']
            measures = [roi_a, roi_b1, roi_b2, roi_c, roi_indirect, roi_direct, roi_simple_indirect, roi_bic1, roi_bic2]
            for measure, title in zip(measures, titles):
                fig = plt.figure(figsize=( (1+(len(rois)*0.15)),1.75))
                locs = np.arange(0,len(rois))
                for r, roi in enumerate(rois):
                ...
                plt.xticks(locs, roi_names, rotation=45)
                plt.tight_layout()
                fig.savefig(os.path.join(self.figure_folder, 'V123', 'cp', 'mediation_{}_{}.pdf'.format(version, title)))
            
            print
            print np.mean(roi_bic1)
            ...
            # # shell()
            # nr_subjects = len(self.subjects)
            # dv = np.concatenate((roi_indirect[:,0], roi_indirect[:,1], roi_indirect[:,2]))
            # area = np.concatenate((np.zeros(nr_subjects), np.ones(nr_subjects)*1, np.ones(nr_subjects)*2,))
            # subject = np.concatenate((np.arange(nr_subjects), np.arange(nr_subjects), np.arange(nr_subjects)))
            # d = rlc.OrdDict([('area', robjects.FactorVector(list(area.ravel()))), ('subject', robjects.FactorVector(list(subject.ravel()))), ('data', robjects.FloatVector(list(dv.ravel())))])
            # robjects.r.assign('dataf', robjects.DataFrame(d))
    ...
    def OLD_CHOICE_SIGNALS_mediation_analysis_2(self):
        ...
        for version in [1]:
            ...
            for roi in rois:
                ...
                if version == 1:
                    ...
                    bbic2 = np.zeros(len(self.subjects))
                    for i in range(len(self.subjects)):
                        M = np.array(self.yes[i], dtype=int)
                        Y = np.array(self.pupil_data[i][X_measure])
                        try:
                            X = np.array(dfs[i][roi])
                        except:
                            X = np.array(dfs_cortex[i][roi])
                        a, b1, c_p, c, bic1, bic2 = myfuncs.mediation_analysis(X,Y,M, eq1='logit', eq2='ols', simple=True)
                        simple_indirect[i] = a*b1
                        # a, b1, b2, c_p, c, bic1, bic2 = myfuncs.mediation_analysis(X,Y,M, eq1='logit', eq2='ols', simple=False)
                        # bb2[i] = b2
                        # indirect[i] = a*(b1+b2)
                        direct[i] = c
                        aa[i] = a
            ...
            for measure, title in zip(measures, titles):
                ...
                plt.xticks(locs, roi_names, rotation=45)
                plt.tight_layout()
                fig.savefig(os.path.join(self.figure_folder, 'V123', 'cp', 'mediation_2_{}_{}.pdf'.format(version, title)))
            
            print
            print np.mean(roi_bic1)
    
    def OLD_MULTIVARIATE_make_dataframe(self, data_type, prepare=False):
        ...
        # params:
        # rois = ['insula', 'ips_anterior', 'ips_posterior', 'parietal', 'postcentral_sulcus', 'precentral_sulcus', 'parietal']
        rois = ['lr_aIPS', 'lr_PCeS', 'lr_PCeS_s', 'lr_M1', 'lr_pInsula',]
        nr_voxels_for_lat = 50 # means 50 per hemisphere, so 100 in total.
        nr_voxels_for_V123 = 100 # same N-voxels as choice selective
        nr_voxels_for_multivariate = 100 # to really only look at the most selective voxels.
        v = 1
        ...
        if prepare:
            for i in range(len(self.subjects)):
                ...
                # scalars = np.array(NiftiImage(os.path.join(self.data_folder, 'event_related_average_wb', 'scalars_{}_{}_{}_{}.nii.gz'.format(data_type, time_locked, self.subjects[i], type_response))).data)
                # scalars_flipped = scalars[:,:,:,::-1]
                # scalars_control = (scalars[:,np.array(NiftiImage(os.path.join('/home/shared/Niels_UvA/surface_masks/', 'rh.precentral_sulcus_control.nii.gz')).data, dtype=bool)+np.array(NiftiImage(os.path.join('/home/shared/Niels_UvA/surface_masks/', 'lh.precentral_sulcus_control.nii.gz')).data, dtype=bool)].mean(axis=1)) / 2.0
                # # scalars_control = scalars[:,epi_box*cortex_mask].mean(axis=1)
                # # scalars_whole_brain = vstack([scalars[:,np.array(NiftiImage(os.path.join('/home/shared/Niels_UvA/surface_masks/', 'rh.'+r+'.nii.gz')).data, dtype=bool)+np.array(NiftiImage(os.path.join('/home/shared/Niels_UvA/surface_masks/', 'lh.'+r+'.nii.gz')).data, dtype=bool)].mean(axis=1) for r in rois]).mean(axis=0)
                # np.save(os.path.join(self.data_folder, 'choice_areas', 'scalars_control_{}.npy'.format(self.subjects[i])), scalars_control)
                # for r in ['lr_aIPS', 'lr_PCeS', 'lr_PCeS_s', 'lr_M1', 'lr_aIPS_s1', 'lr_aIPS_s2', 'lr_PCeS_s1', 'lr_PCeS_s2', 'lr_M1_s1', 'lr_M1_s2', 'lr_pInsula_s1', 'FS_G_parietal_sup', 'FS_S_intrapariet_and_P_trans', 'lr_aIPS_TPR', 'lr_PCeS_TPR', 'lr_pInsula_TPR']:
                #     if r in ['FS_G_parietal_sup', 'FS_S_intrapariet_and_P_trans']:
                #         subtract = np.sum(np.vstack([np.array(NiftiImage(os.path.join('/home/shared/Niels_UvA/surface_masks/', 'rh.'+rr+'.nii.gz')).data, dtype=bool)[np.newaxis,...] for rr in ['lr_aIPS_s1', 'lr_aIPS_s2', 'lr_PCeS_s1', 'lr_PCeS_s2', 'lr_M1_s1', 'lr_M1_s2', 'lr_pInsula_s1']]), axis=0, dtype=bool)
                #         roi_rh = np.array(NiftiImage(os.path.join('/home/shared/Niels_UvA/surface_masks/', 'rh.'+r+'.nii.gz')).data, dtype=bool) & ~subtract
                #     else:
                #         roi_rh = np.array(NiftiImage(os.path.join('/home/shared/Niels_UvA/surface_masks/', 'rh.'+r+'.nii.gz')).data, dtype=bool)
                #     if self.subjects[i] in ['DE', 'DL', 'JG', 'LH', 'LP', 'NS', 'TK']:
                #         scalars_rh = scalars[:,roi_rh]
                #         scalars_lh = scalars_flipped[:,roi_rh]
                #     else:
                ...
                print self.subjects[i]
                scalars = np.array(NiftiImage(os.path.join(self.data_folder, 'event_related_average_wb', 'scalars_{}_{}_{}_RT_{}.nii.gz'.format('clean_MNI', time_locked, self.subjects[i], type_response))).data)
                for r in ['sl_IPL', 'sl_SPL1', 'sl_SPL2', 'sl_pIns']:
                    subtract = np.sum(np.vstack([np.array(NiftiImage(os.path.join('/home/shared/Niels_UvA/surface_masks/', 'lh.'+rr+'.nii.gz')).data, dtype=bool)[np.newaxis,...] for rr in ['lr_aIPS', 'lr_PCeS', 'lr_M1']]), axis=0, dtype=bool)
                    roi_lh = np.array(NiftiImage(os.path.join('/home/shared/Niels_UvA/surface_masks/', 'lh.'+r+'.nii.gz')).data, dtype=bool) & ~subtract
                    subtract = np.sum(np.vstack([np.array(NiftiImage(os.path.join('/home/shared/Niels_UvA/surface_masks/', 'rh.'+rr+'.nii.gz')).data, dtype=bool)[np.newaxis,...] for rr in ['lr_aIPS', 'lr_PCeS', 'lr_M1']]), axis=0, dtype=bool)
                    roi_rh = np.array(NiftiImage(os.path.join('/home/shared/Niels_UvA/surface_masks/', 'rh.'+r+'.nii.gz')).data, dtype=bool)
                    scalars_lh = scalars[:,roi_lh]
                    scalars_rh = scalars[:,roi_rh]
                    np.save(os.path.join(self.data_folder, 'choice_areas', '{}_{}_lh.npy'.format(r, self.subjects[i])), scalars_lh)
            ...
            for i in range(len(self.subjects)):
                ...
                # regress out rt:
                for s in np.unique(session):
                    scalars_control[session==s] = myfuncs.lin_regress_resid(scalars_control[session==s], [rt[session==s]], project_out=False) + scalars_control[session==s].mean()
                
                df = {}
                for r in rois:
                
                    # based on conjuction two sessions -- select N voxels that show maximum lateraliation and positive responses on yes / no trials:
                    try:
                        scalars_lh = np.load(os.path.join(self.data_folder, 'choice_areas', '{}_{}_lh.npy'.format(r, self.subjects[i])))
                        scalars_rh = np.load(os.path.join(self.data_folder, 'choice_areas', '{}_{}_rh.npy'.format(r, self.subjects[i])))
                        
                        # regress out rt:
                        for s in np.unique(session):
                            for v in range(scalars_lh.shape[1]):
                                scalars_lh[session==s, v] = myfuncs.lin_regress_resid(scalars_lh[session==s, v], [rt[session==s]], project_out=False) + scalars_lh[session==s, v].mean()
                            for v in range(scalars_rh.shape[1]):
                                scalars_rh[session==s, v] = myfuncs.lin_regress_resid(scalars_rh[session==s, v], [rt[session==s]], project_out=False) + scalars_rh[session==s, v].mean()
                        
                        t_values_lh = (sp.stats.ttest_1samp(scalars_lh[yes], 0)[0] + (sp.stats.ttest_1samp(scalars_lh[~yes], 0)[0]*-1.0) ) / 2.0
                        t_values_rh = (sp.stats.ttest_1samp(scalars_rh[~yes], 0)[0] + (sp.stats.ttest_1samp(scalars_rh[yes], 0)[0]*-1.0) ) / 2.0
                        lh_ind = np.argsort(t_values_lh)[::-1][:nr_voxels_for_lat]
                        rh_ind = np.argsort(t_values_rh)[::-1][:nr_voxels_for_lat]
                        values_lh = scalars_lh[:,lh_ind].mean(axis=1)
                        values_rh = scalars_rh[:,rh_ind].mean(axis=1)
                        
                        # # shell()
                        # values_lh_lr = myfuncs.lin_regress_resid(values_lh, [rt], project_out=True) # + values_lh.mean()
                        # prj = rt.copy()
                        # prj = prj / np.sqrt((prj ** 2).sum(-1))[..., np.newaxis]
                        # values_lh_op = values_lh - (values_lh.dot(prj)*prj)
                        
                        # # regress out rt:
                        # for s in np.unique(session):
                        #     values_lh[session==s] = myfuncs.lin_regress_resid(values_lh[session==s], [rt[session==s]], project_out=False) + values_lh[session==s].mean()
                        #     values_rh[session==s] = myfuncs.lin_regress_resid(values_rh[session==s], [rt[session==s]], project_out=False) + values_rh[session==s].mean()
                    ...
                    # cross validate:
                    try:
                        scalars_lh_s1 = np.load(os.path.join(self.data_folder, 'choice_areas', '{}_{}_lh.npy'.format(r+'_s1', self.subjects[i])))
                        scalars_rh_s1 = np.load(os.path.join(self.data_folder, 'choice_areas', '{}_{}_rh.npy'.format(r+'_s1', self.subjects[i])))
                        try:
                            scalars_lh_s2 = np.load(os.path.join(self.data_folder, 'choice_areas', '{}_{}_lh.npy'.format(r+'_s2', self.subjects[i])))
                            scalars_rh_s2 = np.load(os.path.join(self.data_folder, 'choice_areas', '{}_{}_rh.npy'.format(r+'_s2', self.subjects[i])))
                            t_values_1_lh = (sp.stats.ttest_1samp(scalars_lh_s1[yes & (session==1)], 0)[0] + (sp.stats.ttest_1samp(scalars_lh_s1[~yes & (session==1)], 0)[0]*-1.0) ) / 2.0
                            t_values_1_rh = (sp.stats.ttest_1samp(scalars_rh_s1[~yes & (session==1)], 0)[0] + (sp.stats.ttest_1samp(scalars_rh_s1[yes & (session==1)], 0)[0]*-1.0) ) / 2.0
                            t_values_2_lh = (sp.stats.ttest_1samp(scalars_lh_s2[yes & (session!=1)], 0)[0] + (sp.stats.ttest_1samp(scalars_lh_s2[~yes & (session!=1)], 0)[0]*-1.0) ) / 2.0
                            t_values_2_rh = (sp.stats.ttest_1samp(scalars_rh_s2[~yes & (session!=1)], 0)[0] + (sp.stats.ttest_1samp(scalars_rh_s2[yes & (session!=1)], 0)[0]*-1.0) ) / 2.0
                            indices_1_lh = np.argsort(t_values_1_lh)[::-1][:nr_voxels_for_lat]
                            indices_1_rh = np.argsort(t_values_1_rh)[::-1][:nr_voxels_for_lat]
                            indices_2_lh = np.argsort(t_values_2_lh)[::-1][:nr_voxels_for_lat]
                            indices_2_rh = np.argsort(t_values_2_rh)[::-1][:nr_voxels_for_lat]
                            s_lh_1 = (scalars_lh_s2[session==1,:][:,indices_2_lh]).mean(axis=1)
                            s_rh_1 = (scalars_rh_s2[session==1,:][:,indices_2_rh]).mean(axis=1)
                            s_lh_2 = (scalars_lh_s1[session!=1,:][:,indices_1_lh]).mean(axis=1)
                            s_rh_2 = (scalars_rh_s1[session!=1,:][:,indices_1_rh]).mean(axis=1)
                            values_lh = np.concatenate((s_lh_1, s_lh_2))
                            values_rh = np.concatenate((s_rh_1, s_rh_2))
                            ...
                            values_mean = myfuncs.lin_regress_resid(values_mean, [scalars_control], project_out=True)
                            values_lat = values_lh - values_rh
                            df[r+'_cv'] = values_mean
                            df[r+'_cv_lat'] = values_lat
                            df[r+'_cv_lh'] = values_lh
                            df[r+'_cv_rh'] = values_rh
                        except:
                            ...
                            indices_1_lh = np.argsort(t_values_1_lh)[::-1][:nr_voxels_for_lat]
                            indices_1_rh = np.argsort(t_values_1_rh)[::-1][:nr_voxels_for_lat]
                            values_lh = (scalars_lh_s1[session!=1,:][:,indices_1_lh]).mean(axis=1)
                            values_rh = (scalars_rh_s1[session!=1,:][:,indices_1_rh]).mean(axis=1)
                            
                            # regress out rt:
                            for s in np.unique(session):
                                values_lh = myfuncs.lin_regress_resid(values_lh, [rt[session!=1]], project_out=False) + values_lh.mean()
                                values_rh = myfuncs.lin_regress_resid(values_rh, [rt[session!=1]], project_out=False) + values_rh.mean()
                            
                            values_mean = (values_lh + values_rh) / 2.0
                            values_mean = myfuncs.lin_regress_resid(values_mean, [scalars_control[session!=1]], project_out=True)
                            values_lat = values_lh - values_rh
                            nan_array = np.zeros(len(session))
                            nan_array[:] = np.NaN
                            dummy = nan_array.copy()
                            dummy[-len(values_mean):] = values_mean
                            df[r+'_cv'] = dummy
                            dummy = nan_array.copy()
                            dummy[-len(values_lat):] = values_lat
                            df[r+'_cv_lat'] = dummy
                            dummy = nan_array.copy()
                            dummy[-len(values_lh):] = values_lh
                            df[r+'_cv_lh'] = dummy
                            dummy = nan_array.copy()
                            dummy[-len(values_rh):] = values_rh
                            df[r+'_cv_rh'] = dummy
                    except:
                        pass
                    # if v == 3:
                    #     # split in two:
                    #     # select N voxels that show maximum lateraliation:
                    #     t_values_1_lh = (sp.stats.ttest_1samp(scalars_lh[yes & (session==1)], 0)[0] + (sp.stats.ttest_1samp(scalars_lh[~yes & (session==1)], 0)[0]*-1.0) ) / 2.0
                    #     t_values_1_rh = (sp.stats.ttest_1samp(scalars_rh[~yes & (session==1)], 0)[0] + (sp.stats.ttest_1samp(scalars_rh[yes & (session==1)], 0)[0]*-1.0) ) / 2.0
                    #     t_values_2_lh = (sp.stats.ttest_1samp(scalars_lh[yes & (session!=1)], 0)[0] + (sp.stats.ttest_1samp(scalars_lh[~yes & (session!=1)], 0)[0]*-1.0) ) / 2.0
                    #     t_values_2_rh = (sp.stats.ttest_1samp(scalars_rh[~yes & (session!=1)], 0)[0] + (sp.stats.ttest_1samp(scalars_rh[yes & (session!=1)], 0)[0]*-1.0) ) / 2.0
                    #     indices_1_lh = np.argsort(t_values_1_lh)[::-1][:nr_voxels_for_lat]
                    #     indices_2_lh = np.argsort(t_values_2_lh)[::-1][:nr_voxels_for_lat]
                    #     indices_1_rh = np.argsort(t_values_1_rh)[::-1][:nr_voxels_for_lat]
                    #     indices_2_rh = np.argsort(t_values_2_rh)[::-1][:nr_voxels_for_lat]
                    #     s_lh_1 = (scalars_lh[session==1,:][:,indices_2_lh]).mean(axis=1)
                    #     s_rh_1 = (scalars_rh[session==1,:][:,indices_2_rh]).mean(axis=1)
                    #     s_lh_2 = (scalars_lh[session!=1,:][:,indices_1_lh]).mean(axis=1)
                    #     s_rh_2 = (scalars_rh[session!=1,:][:,indices_1_rh]).mean(axis=1)
                    #     # s_lh_1 = myfuncs.lin_regress_resid(s_lh_1, [scalars_control[session==1]], project_out=True)
                    #     # s_rh_1 = myfuncs.lin_regress_resid(s_rh_1, [scalars_control[session==1]], project_out=True)
                    #     # s_lh_2 = myfuncs.lin_regress_resid(s_lh_2, [scalars_control[session!=1]], project_out=True)
                    #     # s_rh_2 = myfuncs.lin_regress_resid(s_rh_2, [scalars_control[session!=1]], project_out=True)
                    #     values_lh = np.concatenate((s_lh_1, s_lh_2))
                    #     values_rh = np.concatenate((s_rh_1, s_rh_2))
                    
                    # add to dataframe:
                    # for s in np.unique(session):
                    #     values_lat[session==s] = myfuncs.lin_regress_resid(values_lat[session==s], [rt[session==s]])
                ...
                s = []
                for roi in ['V1', 'V2', 'V3']:
                    files = sort(glob.glob(os.path.join(self.data_folder, 'noise_correlation', 'scalars{}_{}*{}_{}_center*'.format(self.postfix, self.subjects[i], roi, nr_voxels_for_V123))))
                    s.append( np.concatenate([np.load(f) for f in files]) )
                    files = sort(glob.glob(os.path.join(self.data_folder, 'noise_correlation', 'scalars{}_{}*{}_{}_surround*'.format(self.postfix, self.subjects[i], roi, nr_voxels_for_V123))))
                    s.append( np.concatenate([np.load(f) for f in files]) )
                scalars_visual = np.hstack(s)
                ...
                for r in ['V1_center', 'V2_center', 'V3_center']:
                    files = sort(glob.glob(os.path.join(self.data_folder, 'V123', 'scalars{}_{}_{}_{}_{}_*.npy'.format(self.postfix, r, ind_type, 'all', self.subjects[i],))))
                    scalars_m = [np.load(f) for f in files]
                ...
                # for r in ['V1', 'V2', 'V3']:
                #
                #     # files_center = sort(glob.glob(os.path.join(self.data_folder, 'V123', 'scalars{}_{}_center_{}_{}_{}_*.npy'.format(self.postfix, r, ind_type, 'all', self.subjects[i],))))
                #     # files_surround = sort(glob.glob(os.path.join(self.data_folder, 'V123', 'scalars{}_{}_surround_{}_{}_{}_*.npy'.format(self.postfix, r, ind_type, 'all', self.subjects[i],))))
                #
                #     files_center = sort(glob.glob(os.path.join(self.data_folder, 'noise_correlation', 'scalars{}_{}*{}_{}_center*'.format(self.postfix, self.subjects[i], r, nr_voxels_for_V123))))
                #     files_surround = sort(glob.glob(os.path.join(self.data_folder, 'noise_correlation', 'scalars{}_{}*{}_{}_surround*'.format(self.postfix, self.subjects[i], r, nr_voxels_for_V123))))
                #
                #     scalars_m_center = [np.load(f) for f in files_center]
                #     scalars_m_surround = [np.load(f) for f in files_surround]
                #     weigthed_mean_output = []
                #     signal_orientation = []
                #     # session = []
                #     trials = 0
                #
                #     scalars_m = [np.hstack((scalars_m_center[s], scalars_m_surround[s])) for s in range(len(files_center))]
                #
                #     for s in range(len(scalars_m)):
                #         # pupil data:
                #         pupil_data = pd.read_csv(os.path.join(self.data_folder[:-6], self.subjects[i], 'pupil_data_{}.csv'.format(s+1)))
                ...
                #         sig_orientation = np.array(pupil_data['signal_orientation'])
                #         present = np.array(pupil_data['present'], dtype=bool)
                #         correct = np.array(pupil_data['correct'], dtype=bool)
                #         yess = np.array(pupil_data['yes'], dtype=bool)
                #
                #         # regress out rt:
                #         for v in range(scalars_m[s].shape[1]):
                ...
                #             trials_for_template[t] = False
                #
                #             X_scaled = scalars_m[s][trials_for_template,:][:,stim_ind]
                #             x_scaled = np.atleast_2d(scalars_m[s][t,:][stim_ind])
                #             y = np.array(yess[trials_for_template], dtype=int)
                #             clf = svm.SVC(kernel='linear',) # class_weight='auto')
                #             clf.fit(X_scaled, y,)
                #
                #             output_s[t] = clf.decision_function(x_scaled)[0][-1]
                #             # output_s[t] = clf.predict(scalars_scaled[t,:])[0]
                #
                #         weigthed_mean_output.append(output_s)
                #         signal_orientation.append(sig_orientation)
                #         trials = trials+nr_trials_in_session
                #     weigthed_mean_output = np.concatenate(weigthed_mean_output)
                #     # mean_output = np.concatenate(mean_output)
                #     df['{}_SVM'.format(r,)] = weigthed_mean_output
                #     # df['{}_mean'.format(r,)] = mean_output
                #     # df['ori'] = np.concatenate(signal_orientation)
                #     # df['session'] = np.concatenate(session)
                #
                # shell()
                ...
                for r in ['sl_IPL', 'sl_SPL1', 'sl_SPL2', 'sl_pIns']:
                    ...
                    scalars_rh = np.load(os.path.join(self.data_folder, 'choice_areas', '{}_{}_rh.npy'.format(r, self.subjects[i])))
                    scalars = np.hstack((scalars_lh, scalars_rh))
                    t_values = sp.stats.ttest_1samp(scalars[yes,:], 0)[0] - sp.stats.ttest_1samp(scalars[~yes,:], 0)[0]
                    # choice_ind = np.argsort(t_values)
                    choice_ind = np.concatenate((np.argsort(t_values)[-nr_voxels_for_lat:][::-1], np.argsort(t_values)[:nr_voxels_for_lat:][::-1]))
                    nr_trials = scalars.shape[0]
                    
                    # decoding:    
                    info_signal = np.zeros(nr_trials)
                    SVM_signal = np.zeros(nr_trials)
                    y = yes.copy()
                    data = scalars[:,:]
                    cv = LeaveOneOut(y.size)
                    # cv = KFold(y.size, n_folds=8)
                    for train, test in cv:
                        ...
                        # template = train_data[y[train],:].mean(axis=0) - train_data[~y[train],:].mean(axis=0)
                        template = sp.stats.ttest_1samp(train_data[y[train],:], 0)[0] - sp.stats.ttest_1samp(train_data[~y[train],:], 0)[0]
                        info_signal[test] = sp.stats.pearsonr(template, test_data.ravel())[0]
                        
                        # SVM:
                        clf = svm.SVC(kernel='linear', class_weight='balanced',) # verbose=1,)
                        clf.fit(train_data, y[train],)
                        SVM_signal[test] = clf.decision_function(test_data)
                    
                    # mean signal:
                    mean_signal = scalars.mean(axis=1)
                    
                    # add:
                    df['{}_mean'.format(r,)] = mean_signal
                    df['{}_info'.format(r,)] = info_signal
                    df['{}_SVM'.format(r,)] = SVM_signal
                    
                # for r in ['lr_aIPS_TPR', 'lr_PCeS_TPR', 'lr_pInsula_TPR']:
                #     scalars_lh = np.load(os.path.join(self.data_folder, 'choice_areas', '{}_{}_lh.npy'.format(r, self.subjects[i])))
                #     scalars_rh = np.load(os.path.join(self.data_folder, 'choice_areas', '{}_{}_rh.npy'.format(r, self.subjects[i])))
                #     # regress out rt:
                #     for s in np.unique(session):
                ...
                #         for v in range(scalars_rh.shape[1]):
                #             scalars_rh[session==s, v] = myfuncs.lin_regress_resid(scalars_rh[session==s, v], [rt[session==s]], project_out=False) + scalars_rh[session==s, v].mean()
                #     values_lh = scalars_lh[:,:].mean(axis=1)
                #     values_rh = scalars_rh[:,:].mean(axis=1)
                #     values_mean = (values_lh + values_rh) / 2.0
                #     values_mean = myfuncs.lin_regress_resid(values_mean, [scalars_control], project_out=True)
                #     values_lat = values_lh - values_rh
                #     df[r] = values_mean
